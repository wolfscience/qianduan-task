 ...
 */
function drawArc(radarR_in) {  //画圈

    var count = (arguments.length==1 && 1*arguments[0]) || (arguments.length==2 && 1*arguments[1])
        || (window.localStorage && 1*window.localStorage.getItem('arcCount')) || 6;
    var radarR_in = arguments[0]||30000;
    if (count != 0) {
        var canvasId = (arguments.length>0 && !(1*arguments[0]) && arguments[0]) || 'arcCanvas';
